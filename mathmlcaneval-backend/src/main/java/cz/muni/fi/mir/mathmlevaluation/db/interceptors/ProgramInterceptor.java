...
public class ProgramInterceptor
{
    ...
    @After("execution(* cz.muni.fi.mir.mathmlevaluation.db.service.ProgramService.createProgram(..)) && args(program)")
    ...
    @Before("execution(* cz.muni.fi.mir.mathmlevaluation.db.service.ProgramService.deleteProgram(..)) && args(program)")
    ...
    @Before("execution(* cz.muni.fi.mir.mathmlevaluation.db.service.ProgramService.updateProgram(..)) && args(program)")
    public void aroundUpdateProgram(Program program)
    {
        databaseEventService.createDatabaseEvent(databaseEventFactory
                .newInstance(DatabaseEvent.Operation.UPDATE, 
                        program, 
                        "Updated program "+program.getId().toString()
                )
        );
    }
}
